...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER("ميوزك بلاك").info(f"جارِ تشغيل الحساب المساعد . . .")
        if config.STRING1:
            ...
            LOGGER("ميوزك بلاك").info(f"تم بدء تشغيل الحساب المساعد {self.one.name} ...✓")

        if config.STRING2:
            await self.two.start()
            try:
                await self.one.join_chat("kkc8c")
                await self.one.join_chat("ceccr")
            except:
                pass
            assistants.append(2)
            try:
            ...
            LOGGER("ميوزك بلاك").info(f"Assistant Two Started as {self.two.name}")
        ...
        if config.STRING3:
            ...
            LOGGER("ميوزك بلاك").info(f"Assistant Three Started as {self.three.name}")
        ...
        if config.STRING4:
            ...
            LOGGER("ميوزك بلاك").info(f"Assistant Four Started as {self.four.name}")

        if config.STRING5:
            await self.five.start()
            try:
                await self.one.join_chat("kkx8c")
                await self.one.join_chat("cecrr")
            except:
                pass
            assistants.append(5)
            ...
            LOGGER("ميوزك بلاك").info(f"Assistant Five Started as {self.five.name}")

    async def stop(self):
        LOGGER("ميوزك بلاك").info(f"Stopping Assistants...")
        try:
            if config.STRING1:
                await self.one.stop()
            if config.STRING2:
